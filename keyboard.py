...
def _set_static_mode(zone, red, green, blue):
    if zone == 0:
        for i in range(1, 5):
            _write_to_device(CHARACTER_DEVICE_STATIC, _create_static_payload(i, red, green, blue))
    else:
        _write_to_device(CHARACTER_DEVICE_STATIC, _create_static_payload(zone, red, green, blue))
